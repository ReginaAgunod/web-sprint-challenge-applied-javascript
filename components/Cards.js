...
// https://lambda-times-api.herokuapp.com/articles Study the response data you
// get back, closely. You will be creating a card for each article in the
// response. This won't be as easy as just iterating over an array though.
//
...
//
// Use your function to create a card for each of the articles, and append each
// card to the DOM.
...
axios.get("https://lambda-times-api.herokuapp.com/articles").then(response => {
  ...
  tabs[0].addEventListener("click", e => {
    ...
    response.data.articles.javascript.forEach(
        item => { cardContainer.appendChild(ArticleCreator(item)); });
  });
  ...
  tabs[2].addEventListener("click", e => {
    cardContainer.innerHTML = "";
    response.data.articles.technology.forEach(
        item => { cardContainer.appendChild(ArticleCreator(item)); });
  });
  tabs[3].addEventListener("click", e => {
    cardContainer.innerHTML = "";
    response.data.articles.jquery.forEach(
        item => { cardContainer.appendChild(ArticleCreator(item)); });
  });
  tabs[4].addEventListener("click", e => {
    cardContainer.innerHTML = "";
    response.data.articles.node.forEach(
        item => { cardContainer.appendChild(ArticleCreator(item)); });
  });
});
